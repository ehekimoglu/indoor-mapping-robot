...
def completion(counter, wall_length, turn_list, turn_angle):
    robot.stop()
    t = turtle.Turtle()
    print(len(wall_length))
    print(len(turn_list))
    for i in range(len(wall_length)):
        print("calculating wall" ,i)
...
#        time.sleep(0.2)
    while  get_dist(f_trig,f_echo)<=400 and 30 <= get_dist(f_trig,f_echo) and not right_wall_detected:
        ...
        robot.forward(0.7, curve_right = curve)
        time.sleep(0.2)
...
while counter == 0:
    ...
    while right_wall_detected and counter == 0:
        ...
        if RightDistance <= 10 and 3 <= end_time - away_time:
            move_away()
            away_time = time.time()
        if 40 <=RightDistance and 2.8 <= end_time - closer_time:
            move_closer()
...
#            next_wall_distance = get_dist(r_trig, f_trig)
            robot.stop()
            right_wall_detected = False
            robot.left(1)
            time.sleep(0.25)
...
#            turn_list.append(right)
#            distance = 0
        if 30 < time.time() - next_time: 
           counter =  completion(counter, wall_length, turn_list, turn_angle)
        continue  
